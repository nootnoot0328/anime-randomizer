...
buildPrompt=function(){
  ...
  if(STATE.lang==="zh"){
    const lines=assignments.map(a=>`${traitLabel(a.trait)}：${promptCharacterName(a.character)}（${promptSeriesName(a.character)||"未知作品"}）`).join("\n");
  ...
  if(STATE.lang==="ja"){
    const lines=assignments.map(a=>`${traitLabel(a.trait)}：${promptCharacterName(a.character)}（${promptSeriesName(a.character)||"不明な作品"}）`).join("\n");
  ...
  const lines=assignments.map(a=>`${a.trait}: ${promptCharacterName(a.character)} (${promptSeriesName(a.character)||"Unknown series"})`).join("\n");
...
buildPKJudgePrompt=function(){
  ...
  if(STATE.lang==="zh")return `判断以下两支动漫队伍的对战结果。以角色正史能力、当前所选时期/形态、速度、耐久、头脑、克制关系、定位和团队配合为依据。跨作品能力体系请做合理等效，不要虚构能力。\n\n定位不是装饰。必须判断：领袖的指挥与胜利条件、副手的支援与接替、前锋／肉盾的保护与战线控制、治疗／救援位的续航、策略位的情报与克制。若存在内鬼位，必须把其破坏计入原队伍。角色单体更强但不适合定位，不能自动获胜；空缺定位代表人数及功能缺失，但不自动判负。最终胜负应以定位履行、克制、团队配合和可执行的胜利条件为主，纯战力只是一项因素。\n\n${block(1)}\n\n${block(2)}\n\n回答要短，只按以下格式：\n结果：玩家1胜 / 玩家2胜 / 平局\n差距：碾压 / 明显优势 / 中等优势 / 接近 / 极其接近 / 势均力敌\n理由：2～4句，说明关键定位如何决定胜负。\n最大因素：1句。\n\n如果是平局，差距写“势均力敌”。`;
  if(STATE.lang==="ja")return `以下の2チームの対戦結果を判定してください。正史の能力、選択中の時期・形態、速度、耐久、知力、相性、役割、チーム連携を基準にしてください。作品をまたぐ能力体系は合理的に比較し、存在しない能力は追加しないでください。\n\n役割は飾りではありません。リーダーの指揮と勝利条件、副官の支援と代行、前衛／タンクの護衛と戦線維持、回復／救助役の継戦能力、戦略役の情報と対策を必ず評価してください。裏切り枠がある場合、その妨害を元チームの不利益として扱います。単体戦闘力が高くても役割不適合なら自動的に勝たせず、空き枠は人数と機能の不足として扱いますが自動敗北にはしません。最終判定は役割遂行、相性、連携、実行可能な勝利条件を優先し、純粋な戦闘力は一要素に留めてください。\n\n${block(1)}\n\n${block(2)}\n\n短く、次の形式だけで回答：\n結果：プレイヤー1勝 / プレイヤー2勝 / 引き分け\n差：圧勝 / 明確な優勢 / 中程度の優勢 / 接戦 / 紙一重 / 五分\n理由：2〜4文で、どの役割が勝敗を決めたか説明。\n最大要因：1文。\n\n引き分けの場合は「差：五分」とする。`;
  return `Judge the battle between these two anime teams using canon abilities, the selected era/form, speed, durability, intelligence, counters, role fit and team synergy. Make reasonable cross-series assumptions and do not invent abilities.\n\nRoles are tactical responsibilities, not labels. Evaluate the leader's command and win condition, the deputy's support and succession, the frontline/tank's protection and battlefield control, the healer/rescue role's sustain, and the strategist's information and counters. If a betrayal role exists, count its sabotage against its original team. A stronger character who cannot perform the assigned role does not automatically win. An empty role is a numerical and functional disadvantage, but not an automatic loss. Decide primarily through role execution, counters, coordination and achievable win conditions; raw power is only one factor.\n\n${block(1)}\n\n${block(2)}\n\nKeep the answer short and use exactly this format:\nRESULT: Player 1 wins / Player 2 wins / Draw\nMARGIN: Overwhelming / Clear / Moderate / Close / Razor-thin / Even\nWHY: 2–4 short sentences explaining which roles decide the battle.\nBIGGEST FACTOR: 1 sentence.\n\nFor a draw, use MARGIN: Even.`;
};
...
buildPKJudgePrompt=function(){
  ...
  const budget=STATE.pk?.kind==="budget";
  if(STATE.lang==="zh"){
    const setup="全明星战斗场景："+scene.zh+"\n场景只负责提供电影感舞台，不得让任何作品自动获得主场优势。\n\n"+(budget?"":"内鬼规则：内鬼、叛忍、叛徒、诅咒师或鬼方叛徒是强制生效的对局规则，不是忠诚的第六位队员。该角色必须在合理的关键时刻背叛自己被分配的队伍；正史性格与能力只决定其保留支援、误导、破坏或倒戈的方式与时机。即使没有改变胜负，也必须比较两队内鬼位对己方造成的影响。\n\n");
    base=base.replace("不要虚构能力。\n\n","不要虚构能力。\n\n"+setup);
    return budget?base:base.replace("理由：2～4句，说明关键定位如何决定胜负。","理由：2～4句，说明关键定位如何决定胜负；其中至少一句必须比较两队内鬼位的影响。");
  }
  if(STATE.lang==="ja"){
    const setup="オールスターバトルの舞台："+scene.ja+"\n舞台は演出のための中立フィールドとし、特定作品に自動的なホームアドバンテージを与えないでください。\n\n"+(budget?"":"裏切り枠のルール：裏切り者、抜け忍、離反者、呪詛師、鬼側の裏切り者は必ず発動する対戦ルールであり、忠実な6人目ではありません。そのキャラは合理的な決定的瞬間に、ドラフトされた自チームを必ず裏切ります。原作の性格と能力は、支援拒否、誤誘導、妨害、寝返りの方法とタイミングを決めるために使ってください。勝敗が変わらなくても、両チームの裏切り枠が味方へ与える影響を必ず比較してください。\n\n");
    base=base.replace("存在しない能力は追加しないでください。\n\n","存在しない能力は追加しないでください。\n\n"+setup);
    return budget?base:base.replace("理由：2〜4文で、どの役割が勝敗を決めたか説明。","理由：2〜4文で、どの役割が勝敗を決めたか説明し、少なくとも1文で両チームの裏切り枠の影響を比較する。");
  }
  ...
  return budget?base:base.replace("WHY: 2–4 short sentences explaining which roles decide the battle.","WHY: 2–4 short sentences explaining which roles decide the battle. At least one sentence must compare the impact of both teams' betrayal slots.");
};
