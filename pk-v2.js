...
function pkJudgeTeamBlock(n){
  const pk=STATE.pk,p=pk[`p${n}`],series=getSeries(p.seriesId),label=STATE.lang==="zh"?`玩家${n}`:STATE.lang==="ja"?`プレイヤー${n}`:`PLAYER ${n}`;
  const rows=p.roles.map(role=>{const slot=p.team.find(x=>x.role===role);if(!slot)return `- ${roleLabel(role)}: ${STATE.lang==="zh"?"空缺":STATE.lang==="ja"?"空き":"[empty]"}`;const c=slot.character;return `- ${roleLabel(role)}: ${promptCharacterName(c)} (${promptSeriesName(c)||displaySeries(series)})`;});
  return `${label} — ${displaySeries(series)}\n${rows.join("\n")}`;
}
function pkJudgeMatchupRows(){
  const pk=STATE.pk,lines=[];for(let i=0;i<Math.min(5,pk.p1.roles.length,pk.p2.roles.length);i++){const a=pk.p1.team.find(x=>x.role===pk.p1.roles[i]),b=pk.p2.team.find(x=>x.role===pk.p2.roles[i]),left=a?promptCharacterName(a.character):(STATE.lang==="zh"?"空缺":STATE.lang==="ja"?"空き":"[empty]"),right=b?promptCharacterName(b.character):(STATE.lang==="zh"?"空缺":STATE.lang==="ja"?"空き":"[empty]");lines.push(`${i+1}. ${roleLabel(pk.p1.roles[i])} vs ${roleLabel(pk.p2.roles[i])}: ${left} vs ${right}`);}return lines.join("\n");
}
function pkJudgeBetrayalText(){
  const pk=STATE.pk;if(pk.kind==="budget")return"";const a=pk.p1.team.find(x=>/:traitor$/.test(x.role)),b=pk.p2.team.find(x=>/:traitor$/.test(x.role)),an=a?promptCharacterName(a.character):(STATE.lang==="zh"?"玩家1内鬼位":STATE.lang==="ja"?"プレイヤー1の裏切り枠":"Player 1's betrayal slot"),bn=b?promptCharacterName(b.character):(STATE.lang==="zh"?"玩家2内鬼位":STATE.lang==="ja"?"プレイヤー2の裏切り枠":"Player 2's betrayal slot");
  if(STATE.lang==="zh")return `本局背叛规则：${an}必须在合理的关键时刻背叛玩家1；${bn}必须以符合正史性格和能力的方式背叛玩家2。比较两人分别对原队伍造成的实际破坏。`;
  if(STATE.lang==="ja")return `この対戦の裏切りルール：${an}は合理的な決定的瞬間にプレイヤー1を、${bn}は原作の性格と能力に沿った方法でプレイヤー2を必ず裏切る。両者が元チームへ与える実害を比較すること。`;
